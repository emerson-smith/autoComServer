...
const generateEmailBody = asyncHandler(async (req, res, next) => {
	...
	// Create prompts
	const systemPrompt =
		'You are a email writing assistant for sales professionals. You help write clear, direct, concise emails for the user. The user will show you the email they have written so far, and you will help write it. The user will mark the location of where you\'ll be writing with the characters "[WRITE_HERE]". Mimic the style and tone of the user. Do not rewrite the email. Do not repeat any ideas already expressed in the email. Do not write the full email. Do not return anything other than what additional text you are suggesting. Do not write [WRITE_HERE]. Do not rewrite the entire email. Only return the characters that will be inserted.';
	let prompt = "";
	let initialPrompt = "";
	let lineStop = true;
	if (formData.typeOfCompletion === "paragraph" && formData.bodyAfter.length < 6) {
		...
		initialPrompt = `In this email, write a short paragraph that will be inserted into the email at the [WRITE_HERE] tag. Only write 1 paragraph. Your response will be replacing the [WRITE_HERE] marker, so do not respond with it. \n\nHere is the email:\n---\n`;
		lineStop = false;
	} else if (formData.typeOfCompletion === "line") {
		initialPrompt = `In this email, continue writing at the location of the [WRITE_HERE] marker. Your response will be replacing the [WRITE_HERE] marker, so do not respond with it. \n\nHere is the email:\n---\n`;
		lineStop = true;
	}
